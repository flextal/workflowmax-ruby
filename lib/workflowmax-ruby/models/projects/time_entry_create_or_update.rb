...
module WorkflowMaxRuby::Projects
  require 'bigdecimal'
  ...
  class TimeEntryCreateOrUpdate
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorkflowMaxRuby::Projects::TimeEntryCreateOrUpdate` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorkflowMaxRuby::Projects::TimeEntryCreateOrUpdate`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def _deserialize(type, value)
      ...
      when /\AHash<(?<k_type>.+?), (?<v_type>.+)>\z/
        ...
        WorkflowMaxRuby::Projects.const_get(type).build_from_hash(value)
      end
    end
