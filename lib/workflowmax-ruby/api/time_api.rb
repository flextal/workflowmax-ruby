module WorkflowMaxRuby
  class TimeApi
    ...
    # @return [Clients]
    def get_times(xero_tenant_id, opts = {})
      data, _status_code, _headers = get_times_with_http_info(xero_tenant_id, opts)
      data
    end
    ...
    def get_timesheets_with_http_info(xero_tenant_id, options = {})
      ...
      # return_type
      return_type = opts[:return_type] || 'Times'
      ...
      data, status_code, headers = @api_client.call_api(:GET, local_var_path, "TimeApi", new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: TimeAPI#get_times\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
