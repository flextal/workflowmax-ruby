  ...
  class QuoteApi
    ...
    def get_quotes_with_http_info(xero_tenant_id, options = {})
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: QuoteApi.get_quotes ...'
      end
      ...
      data, status_code, headers = @api_client.call_api(:GET, local_var_path, "QuoteApi", new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: QuoteAPI#get_quotes\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
