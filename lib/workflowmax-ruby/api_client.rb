...
module WorkflowMaxRuby
  class ApiClient
    ...
    def accounting_api
      ...
      WorkflowMaxRuby::AccountingApi.new(self)
    end

    def asset_api
      @config.base_url = @config.asset_url
      WorkflowMaxRuby::AssetApi.new(self)
    end

    def project_api
      @config.base_url = @config.project_url
      WorkflowMaxRuby::ProjectApi.new(self)
    end

    def files_api
      @config.base_url = @config.files_url
      WorkflowMaxRuby::FilesApi.new(self)
    end

    def client_api
      @config.base_url = @config.client_url
      WorkflowMaxRuby::ClientApi.new(self)
    end

    def invoice_api
      @config.base_url = @config.invoice_url
      WorkflowMaxRuby::InvoiceApi.new(self)
    end

    def job_api
      @config.base_url = @config.job_url
      WorkflowMaxRuby::JobApi.new(self)
    end

    def task_api
      @config.base_url = @config.task_url
      WorkflowMaxRuby::TaskApi.new(self)
    end

    def quote_api
      @config.base_url = @config.quote_url
    ...
    def call_api(http_method, path, api_client, opts = {})
      ...
      when "ClientApi"
        method_base_url = @config.client_url
      when "InvoiceApi"
        method_base_url = @config.invoice_url
      when "JobApi"
    ...
    def convert_to_type(data, return_type, api_client)
      ...
      when /\AHash\<String, (.+)\>\z/
        ...
        when 'AssetApi'
          WorkflowMaxRuby::Assets.const_get(return_type).build_from_hash(data)
        when 'ProjectApi'
          WorkflowMaxRuby::Projects.const_get(return_type).build_from_hash(data)
        when 'FilesApi'
        ...
        when 'InvoiceApi'
          WorkflowMaxRuby::Invoices.const_get(return_type).build_from_hash(data)
        when 'TaskApi'
